...
class Chart2ViewC: BaseViewController<ChartView> {
    //    override init() {
    //        super.init()
    //    }
    var chosenBank = [Bank]()
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        self.navigationItem.title = "Kryteria"
        for i in 0..<chosenBank.count{
            let row = BankRating(name: chosenBank[i].name, value: s0[i]*100)
            banks.append(row)
        }
        ...
        self.aView.xAxis.valueFormatter = IndexAxisValueFormatter(values: ["Oprocentowanie","Raty","Prowizja"])
    ...
    func saveOption(){
        self.navigationController?.pushViewController(ChartViewController.init(ranking: rankingGlobalny, banks: chosenBank, s0: s0), animated: true)
    }
