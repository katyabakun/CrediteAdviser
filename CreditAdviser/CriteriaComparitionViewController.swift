//
//  CriteriaComparitionViewController.swift
//  CreditAdviser
//
...
struct Item{
    ...
    init(nameA: String,nameB:String,options: [String:Int],imageA:UIImage,imageB:UIImage) {
        self.nameA=nameA
        self.nameB=nameB
...
class CriteriaComparitionViewController: UITableViewController{
    ...
    init(initWithData banks: [Bank],s0: [Double],navTitle: String){
...
extension CriteriaComparitionViewController{
...
//            }
            ...
            let isSpojne = czySpojne(wekt_pref: wekt_preferencji(macierz: macierz_unorm(macierz: matrix)), suma_kol: sumaKolumn(macierz: matrix))
                if isSpojne{
                    self.navigationController?.pushViewController(MiddleCriteriaComparitionViewController.init(initWithData: chosenBank, s0: s0, s1: s1,navTitle: rows[1]), animated: true)
                }else{
                    ...
                    let zmienAction = UIAlertAction(title: "Zmień", style: .default) { (_) -> Void in
                        ...
                        let okButton = UIAlertAction(title: "Tak", style: .default) { (_) -> Void in
                            ...
                            self.navigationController?.pushViewController(MiddleCriteriaComparitionViewController.init(initWithData: self.chosenBank, s0: self.s0, s1: self.s1,navTitle: self.rows[1]), animated: true)
                        }
...
//                    let alertController = UIAlertController(title: "Zmień macierz porównań", message:
//                        "Macierz nie jest spójna!", preferredStyle: UIAlertControllerStyle.alert)
//                    alertController.addAction(UIAlertAction(title: "Zmień", style: UIAlertActionStyle.default,handler: nil))
//                    let action = UIAlertAction(title: "Zatwierdź", style: .default) { (_) -> Void in
//                        
//                        self.navigationController?.pushViewController(MiddleCriteriaComparitionViewController.init(initWithData: self.chosenBank, s0: self.s0, s1: self.s1,navTitle: self.rows[1]), animated: true) }
//                    alertController.addAction(action)
